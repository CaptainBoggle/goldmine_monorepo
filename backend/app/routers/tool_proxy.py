...
from goldmine.types import ToolInput, ToolInfo, ToolResponse, ToolStatus, LoadResponse
...
async def predict_with_tool(tool_id: str, input_data: ToolInput, tool_service: ToolService = Depends(get_tool_service)):
...
async def _proxy_get_request(base_url: str, endpoint: str) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            raise HTTPException(status_code=e.response.status_code, detail=f"Tool returned error: {e.response.text}")
